...
const ACTIVITY_HISTORY_PATH = path_1.default.resolve(__dirname, "activityHistory.json");
const ERC20_ABI = [
...
function printBanner() {
    console.log(chalk_1.default.cyan(`\n==============================`));
    console.log(chalk_1.default.cyan(`      Vynix Automation Bot   `));
    console.log(chalk_1.default.cyan(`    Bot v1.0 made by JustineDevs `));
    console.log(chalk_1.default.cyan(`==============================\n`));
}
// Logging helpers
const log = {
...
async function swapSettingsMenu() {
    while (true) {
        ...
        if (swapAction === "Set Random Interval") {
            ...
            const { useFixed } = await inquirer_1.default.prompt({
                type: "confirm",
                name: "useFixed",
            ...
            if (useFixed) {
                ...
                const { amountRange } = await inquirer_1.default.prompt({
                    type: "input",
                    name: "amountRange",
...
async function runSwap({ net, tokens, provider, privateKeys, walletIndex, simulationMode, log, contractExists, toFixedDecimals, ethers, swapCount, swapHistory }) {
    // Router contract
    const routerAbi = [
    ...
    let pairFound = false;
    let fromToken = null, toToken = null, direction = null;
    let amount = 0;
...
function pickRandomPair(tokens) {
    if (tokens.length < 2)
        throw new Error('Need at least 2 tokens');
...
async function getTokenBalance(token, address, provider, ethers) {
...
async function runTransfer({ net, provider, privateKeys, walletIndex, simulationMode, log, receiver }) {
    ...
    if (!toAddress) {
        const { stop } = await inquirer_1.default.prompt([
    ...
    try {
        ...
        if (balance < required) {
            log.warn(`Skipping transfer: Insufficient PHRS balance: ${ethers_1.ethers.formatEther(balance)} < ${amount}`);
            const { stop } = await inquirer_1.default.prompt([
...
async function claimFaucetTS(wallet, log, proxy = null) {
    try {
...
async function performCheckInTS(wallet, log, proxy = null) {
    try {
...
async function runFaucet({ net, provider, privateKeys, walletIndex, simulationMode, log }) {
    if (simulationMode) {
...
async function runCheckin({ net, provider, privateKeys, walletIndex, simulationMode, log }) {
    if (simulationMode) {
...
async function runDeploy({ net, provider, privateKeys, walletIndex, simulationMode, log }) {
    if (simulationMode) {
...
async function mainMenu(networks, tokens, privateKey) {
    while (true) {
        ...
        const simStatus = simulationMode ? chalk_1.default.greenBright('[Simulation Mode: ON]') : chalk_1.default.redBright('[Simulation Mode: OFF]');
        const { action } = await inquirer_1.default.prompt([
            {
                type: "list",
                name: "action",
                ...
                choices: [
                    new inquirer_1.default.Separator("= Automation ="),
                    "Start Automation",
                    "Browser Automation (Brave) Under Development",
                    "MetaMask & Pharos Setup",
                    new inquirer_1.default.Separator("= Settings ="),
                    "Modify Settings",
                    new inquirer_1.default.Separator("= Advanced Features ="),
                    "Profit/Loss Tracking",
                    "Automatic Token Approval",
                    "Health Checks & Self-Healing",
                    new inquirer_1.default.Separator("= Info ="),
                    "Show Info & History",
                    new inquirer_1.default.Separator("= Simulation/Dry Run ="),
                    simulationMode ? "Disable Simulation Mode (Dry Run)" : "Enable Simulation Mode (Dry Run)",
                    new inquirer_1.default.Separator("= Other ="),
                    "Other",
                    "Exit"
                ]
            }
        ...
        if (action === "MetaMask & Pharos Setup") {
            ...
            const { simChoice } = await inquirer_1.default.prompt([
                {
                    type: "confirm",
                    name: "simChoice",
            ...
            log.info(simulationMode ? chalk_1.default.greenBright("Simulation/Dry Run Mode enabled. No real transactions will be sent.") : chalk_1.default.redBright("Simulation/Dry Run Mode disabled. Real transactions will be sent."));
            ...
            simulationMode = true;
            log.success(chalk_1.default.greenBright("Simulation/Dry Run Mode enabled. No real transactions will be sent."));
        }
        else if (action === "Disable Simulation Mode (Dry Run)") {
            simulationMode = false;
            log.success(chalk_1.default.redBright("Simulation/Dry Run Mode disabled. Real transactions will be sent."));
        }
...
async function modifySettingsMenu(networks, tokens, privateKey) {
    while (true) {
...
async function transferSettingsMenu() {
    while (true) {
        const { transferSetting } = await inquirer_1.default.prompt([
            {
                type: "list",
        ...
        if (transferSetting === "Set Default Receiver Address") {
            const { receiver } = await inquirer_1.default.prompt([
            ...
            const { amount } = await inquirer_1.default.prompt([
                { type: "input", name: "amount", message: "Enter main transfer amount:", default: globalTransferAmount !== null ? String(globalTransferAmount) : "0.000001", validate: (v) => !isNaN(Number(v)) && Number(v) > 0 }
            ]);
            const { amountRange } = await inquirer_1.default.prompt([
                { type: "input", name: "amountRange", message: "Enter optional upper range for random transfer amount (leave blank for fixed):", default: globalTransferAmountRange !== null ? String(globalTransferAmountRange) : "", validate: (v) => v === '' || (!isNaN(Number(v)) && Number(v) > 0) }
            ]);
...
async function otherMenu() {
    while (true) {
        const { otherAction } = await inquirer_1.default.prompt([
            {
                type: "list",
                name: "otherAction",
...
async function deployOptionsMenu(networks, tokens, privateKey) {
    while (true) {
        const { deployAction } = await inquirer_1.default.prompt([
            {
                type: "list",
                name: "deployAction",
        ...
        if (deployAction === "Custom Contract Interactions") {
            ...
            const wallet = new ethers_1.ethers.Wallet(privateKey, networks[0] ? new ethers_1.ethers.JsonRpcProvider(networks[0].rpc, networks[0].chainId) : undefined);
            while (true) {
                await deployedContractsMenu(wallet, networks[0] ? new ethers_1.ethers.JsonRpcProvider(networks[0].rpc, networks[0].chainId) : undefined);
                const { cont } = await inquirer_1.default.prompt([
...
let globalTransferReceiver = null;
let globalTransferAmount = null;
let globalTransferAmountRange = null;
// Helper: Send remaining balance to self address (self-drain)
async function selfDrainWallet(walletObj, provider, log) {
    try {
        const wallet = new ethers_1.ethers.Wallet(walletObj.privateKey, provider);
        const balance = await provider.getBalance(wallet.address);
        // Leave a small amount for gas
        const gasPrice = (await provider.getFeeData()).gasPrice || ethers_1.ethers.parseUnits('1', 'gwei');
        ...
        if (balance > minGas) {
            ...
            log && log.warn && log.warn(`Self-drain: Sent ${ethers_1.ethers.formatEther(value)} ETH from ${wallet.address} to itself. Tx: ${tx.hash}`);
...
async function deployedContractsMenu(wallet, provider) {
    while (true) {
        const { contractType } = await inquirer_1.default.prompt([
            {
                type: "list",
                name: "contractType",
        ...
        if (contractType === "back")
            break;
        if (contractType === "ERC20") {
            const contract = new ethers_1.ethers.Contract(DEPLOYED_ADDRESSES.ERC20, CUSTOM_ERC20_ABI, wallet);
            const name = await contract.name();
            const symbol = await contract.symbol();
            const totalSupply = await contract.totalSupply();
            const myBalance = await contract.balanceOf(wallet.address);
            console.log(chalk_1.default.green(`\nCustomERC20: ${name} (${symbol})`));
            console.log(`Total Supply: ${ethers_1.ethers.formatUnits(totalSupply, 18)}`);
            console.log(`Your Balance: ${ethers_1.ethers.formatUnits(myBalance, 18)}`);
            const { erc20Action } = await inquirer_1.default.prompt([
                { type: "list", name: "erc20Action", message: "Action:", choices: ["Transfer", "Show Crypto Word", "Back"] }
            ]);
            if (erc20Action === "Transfer") {
                const { to, amount } = await inquirer_1.default.prompt([
                    { type: "input", name: "to", message: "Recipient address:" },
                    { type: "input", name: "amount", message: "Amount to transfer:" }
                ]);
                const tx = await contract.transfer(to, ethers_1.ethers.parseUnits(amount, 18));
                console.log(chalk_1.default.yellow(`Sent! Tx: ${tx.hash}`));
                await tx.wait();
            }
            else if (erc20Action === "Show Crypto Word") {
                const { index } = await inquirer_1.default.prompt([
                    { type: "input", name: "index", message: "Word index (0-255):", validate: (v) => !isNaN(Number(v)) && Number(v) >= 0 && Number(v) < 256 }
                ]);
                const word = await contract.cryptoWords(index);
                console.log(chalk_1.default.cyan(`Word #${index}: ${word}`));
            }
        }
        else if (contractType === "ERC1155") {
            const contract = new ethers_1.ethers.Contract(DEPLOYED_ADDRESSES.ERC1155, CUSTOM_ERC1155_ABI, wallet);
            const name = await contract.name();
            const intervalWord = await contract.intervalWord();
            console.log(chalk_1.default.green(`\nCustomERC1155: ${name} (Interval: ${intervalWord})`));
            const { erc1155Action } = await inquirer_1.default.prompt([
                { type: "list", name: "erc1155Action", message: "Action:", choices: ["Show Balance", "Show Crypto Word", "Back"] }
            ]);
            if (erc1155Action === "Show Balance") {
                const { tokenId } = await inquirer_1.default.prompt([
                    { type: "input", name: "tokenId", message: "Token ID:", default: "1" }
                ]);
                const bal = await contract.balanceOf(wallet.address, tokenId);
                console.log(chalk_1.default.cyan(`Balance of token ${tokenId}: ${bal}`));
            }
            ...
            const contract = new ethers_1.ethers.Contract(DEPLOYED_ADDRESSES.ERC165, CUSTOM_ERC165_ABI, wallet);
            const desc = await contract.getDescription();
            console.log(chalk_1.default.green(`\nCustomERC165 Description: ${desc}`));
            const { erc165Action } = await inquirer_1.default.prompt([
...
function loadActivityHistory() {
    if (fs_1.default.existsSync(ACTIVITY_HISTORY_PATH)) {
        try {
            return JSON.parse(fs_1.default.readFileSync(ACTIVITY_HISTORY_PATH, "utf-8"));
        }
        catch {
            return [];
        }
...
function saveActivityHistory(history) {
    fs_1.default.writeFileSync(ACTIVITY_HISTORY_PATH, JSON.stringify(history, null, 2));
}
...
const BRAVE_USER_DATA_DIR = `${os_1.default.homedir()}\\AppData\\Local\\BraveSoftware\\Brave-Browser\\User Data`;
...
const AUTOMATION_PROFILE_DIR = `${os_1.default.homedir()}\\AppData\\Local\\BraveSoftware\\Brave-Browser\\BraveAutomation`;
...
async function prepareAutomationProfileAdvanced() {
    if (AUTOMATION_PROFILE_CLEAR_ON_START && fs_extra_1.default.existsSync(AUTOMATION_PROFILE_DIR)) {
        await fs_extra_1.default.remove(AUTOMATION_PROFILE_DIR);
    }
    if (!fs_extra_1.default.existsSync(AUTOMATION_PROFILE_DIR)) {
        // If first run, copy from Default
        const mainProfile = `${os_1.default.homedir()}\\AppData\\Local\\BraveSoftware\\Brave-Browser\\User Data\\Default`;
        if (fs_extra_1.default.existsSync(mainProfile)) {
            await fs_extra_1.default.copy(mainProfile, AUTOMATION_PROFILE_DIR, { overwrite: true });
        }
        else {
            fs_extra_1.default.ensureDirSync(AUTOMATION_PROFILE_DIR);
        }
    }
    return AUTOMATION_PROFILE_DIR;
}
// Helper to log session summary
function logAutomationSummary(success, startTime, errorMsg) {
...
async function browserAutomationMenu() {
    ...
    const { minAmount, maxAmount } = await inquirer_1.default.prompt([
    ...
    await automateSwapOrLiquidity(url, action, Number(minAmount), Number(maxAmount));
    log.success('Browser automation complete.');
}
// --- Playwright MetaMask Integration & Pharos Network Setup ---
const METAMASK_PATH = path_1.default.resolve('./metamask-extension'); // Path to unpacked MetaMask extension
...
async function addPharosNetwork(page) {
    await page.bringToFront();
...
const METAMASK_ZIP_PATH = path_1.default.join(AUTOMATION_PROFILE_DIR, 'metamask.zip');
const METAMASK_UNPACKED_PATH = path_1.default.join(AUTOMATION_PROFILE_DIR, 'metamask');
async function downloadMetaMaskExtension() {
    if (fs_extra_1.default.existsSync(METAMASK_UNPACKED_PATH))
        return METAMASK_UNPACKED_PATH;
    log.loading('Downloading MetaMask extension...');
    return new Promise((resolve, reject) => {
        const file = fs_extra_1.default.createWriteStream(METAMASK_ZIP_PATH);
        https_1.default.get(METAMASK_CRX_DOWNLOAD_URL, response => {
            response.pipe(file);
            file.on('finish', async () => {
                file.close();
                try {
                    await (0, extract_zip_1.default)(METAMASK_ZIP_PATH, { dir: METAMASK_UNPACKED_PATH });
            ...
            fs_extra_1.default.unlinkSync(METAMASK_ZIP_PATH);
            reject(err);
        });
    });
}
async function setupMetaMaskOnboarding(page) {
    await page.waitForSelector('text=Get Started', { timeout: 60000 });
    await page.click('text=Get Started');
    await page.click('text=Import an existing wallet');
    await page.click('text=I Agree');
...
async function isMetaMaskSetup(page) {
...
async function automateSwapOrLiquidity(url, action, minAmount, maxAmount) {
    ...
    (0, child_process_1.spawn)(bravePath, [
...
async function automateWalletConnectAndSign(targetUrl) {
    const tempProfile = path_1.default.join(os_1.default.tmpdir(), `brave-playwright-profile-${Date.now()}`);
    await fs_extra_1.default.ensureDir(tempProfile);
    const browser = await playwright_1.chromium.launchPersistentContext(tempProfile, {
        headless: false,
    ...
    await page.addInitScript(() => {
        ...
        window.addEventListener('focus', (e) => {
            // @ts-ignore
            const target = e.target;
            log('Focus', { tag: target?.tagName, id: target?.id, name: target?.name });
        }, true);
        window.addEventListener('blur', (e) => {
            // @ts-ignore
            const target = e.target;
            log('Blur', { tag: target?.tagName, id: target?.id, name: target?.name });
        }, true);
