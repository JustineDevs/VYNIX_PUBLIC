...
async function getPairPrice(token0: string, token1: string): Promise<number> {
        ...
        if (func === "Swap") {
          ...
          while (true) {
            ...
            while (transferActive) {
              const transferPHRS = async (wallet: any, provider: any, index: number) => {
                try {
                  ...
                  if (balance < required) {
                    log.warn(`Skipping transfer ${index + 1}: Insufficient PHRS balance: ${ethers.formatEther(balance)} < ${amount}`);
                    return;
                  }
          ...
          // Faucet claim logic
          const claimFaucet = async (wallet: any, proxy: string | null = null): Promise<boolean> => {
            try {
          ...
          const performCheckIn = async (wallet: any, proxy: string | null = null): Promise<string | null> => {
            try {
